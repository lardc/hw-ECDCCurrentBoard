...
// Forward functions
uint16_t CC_ItoDAC(float Data, uint16_t RegisterOffset, uint16_t RegisterK)
{
	float K = (float)DataTable[RegisterK] / 1000000;
	float Offset = (float)((int16_t)DataTable[RegisterOffset]);
	
	uint32_t result = (Data) * K + Offset;
...
{
	uint16_t Data = (uint32_t)CC_ItoDAC(Current, REG_DAC_I20MA_SET_OFFSET, REG_DAC_I20MA_SET_K) & ~DAC_CHANNEL_B;
...
{
	uint16_t Data = (uint32_t)CC_ItoDAC(Current, REG_DAC_I20A_SET_OFFSET, REG_DAC_I20A_SET_K) | DAC_CHANNEL_B;
	LL_WriteDACx(Data, GPIO_LDAC1);
	DELAY_MS(15);
...
{
	uint16_t Data = (uint32_t)CC_ItoDAC(Current, REG_DAC_I270A_SET_OFFSET, REG_DAC_I270A_SET_K) & ~DAC_CHANNEL_B;
...
void CC_SetCurrentPulse(float OutData, float Current)
{
	uint16_t Data;
	...
	{
		Data = (OutData == 0) ?
				((uint32_t)OutData & ~DAC_CHANNEL_B)  : (CC_ItoDAC(OutData, REG_DAC_I20MA_SET_OFFSET, REG_DAC_I20MA_SET_K) & ~DAC_CHANNEL_B);
		LL_WriteDACx(Data, GPIO_LDAC1);
	}
	...
	{
		Data = (OutData == 0) ?
				((uint32_t)OutData & ~DAC_CHANNEL_B) : (CC_ItoDAC(OutData, REG_DAC_I200MA_SET_OFFSET, REG_DAC_I200MA_SET_K) & ~DAC_CHANNEL_B);
		LL_WriteDACx(Data, GPIO_LDAC1);
	}
	
	else if(Current <= I_RANGE_2A)
	{
		Data = (OutData == 0) ? ((uint32_t)OutData & ~DAC_CHANNEL_B) : (CC_ItoDAC(OutData, REG_DAC_I2A_SET_OFFSET, REG_DAC_I2A_SET_K) & ~DAC_CHANNEL_B);
		LL_WriteDACx(Data, GPIO_LDAC1);
	}
	
	else if(Current <= I_RANGE_20A)
	{
		Data = (OutData == 0) ? ((uint32_t)OutData | DAC_CHANNEL_B) : (CC_ItoDAC(OutData, REG_DAC_I20A_SET_OFFSET, REG_DAC_I20A_SET_K) | DAC_CHANNEL_B);
		LL_WriteDACx(Data, GPIO_LDAC1);
	}
	else
	{
		Data = (OutData == 0) ? ((uint32_t)OutData & ~DAC_CHANNEL_B) : (CC_ItoDAC(OutData, REG_DAC_I270A_SET_OFFSET, REG_DAC_I270A_SET_K) & ~DAC_CHANNEL_B);
		LL_WriteDACx(Data, GPIO_LDAC2);
	}
}
//---------------------
...
void CC_EnableCurrentChannel(float Current, float EnableFb)
{
	...
	else
	{
		...
		LL_EnableRange270A(false);
		LL_DisableRange270A(false);
		Data = (EnableFb == 1) ? I_ANALOG_270A : (I_ANALOG_270A & I_FB_ENABLE);
		LL_WriteOutReg(Data);
	}
}
//---------------------
